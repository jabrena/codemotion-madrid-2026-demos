Feature: God Analysis API
  ...
  # - Filtering is case-insensitive: the filter character matches the first character of each name
  #   ignoring case (e.g. filter=n and filter=N select the same gods).
  ...
  @acceptance-test
  Scenario: Happy path - Get sum with all three sources filtered by prefix 'n'
  ...
  @acceptance-test
  Scenario: Uppercase filter matches the same names as lowercase
    When the client sends a GET request to "/gods/stats/sum" with query parameters "filter" = "N" and "sources" = "greek,roman,nordic"
    Then the response status code should be 200
    And the response body should contain a JSON object with a "sum" field
    And the value of "sum" should be "78179288397447443426"
