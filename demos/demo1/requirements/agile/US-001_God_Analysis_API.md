...
**I want to** consume God APIs (Greek, Roman & Nordic), filter gods whose names start with a given single-character prefix (first character compared ignoring case), convert each filtered god name into its decimal representation, and return the sum of those values
...
- **Case insensitivity:** The `filter` query parameter is a single character; it is compared to the first character of each god name ignoring case (e.g. `filter=n` and `filter=N` select the same names). This matches the acceptance scenarios in [US-001_god_analysis_api.feature](US-001_god_analysis_api.feature).
