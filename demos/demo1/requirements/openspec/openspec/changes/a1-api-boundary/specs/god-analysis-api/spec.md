...
### Requirement: Validate query parameters at boundary
The API MUST validate `filter` and `sources` query parameters before orchestration starts.

#### Scenario: Missing required filter
- **WHEN** the client calls the endpoint without `filter`
- **THEN** the API responds with `400`
- **AND** content type is `application/problem+json`

### Requirement: Return contract-compliant success shape
The API MUST respond with `GodStatsSumResponse` where `sum` is a decimal string.

#### Scenario: Valid request
- **GIVEN** valid `filter` and `sources`
- **WHEN** aggregation succeeds
